                         ...
                         tag:(long)i
{
    if ((self = [super init])) {
        ...
        if (d) {
            ...
            if (readLength > 0) buffer = [[NSMutableData alloc] initWithLength:readLength];
...
- (NSUInteger)optimalReadLengthWithDefault:(NSUInteger)defaultValue shouldPreBuffer:(BOOL *)shouldPreBufferPtr
{
    ...
    if (readLength > 0) {
        ...
        if (shouldPreBufferPtr) *shouldPreBufferPtr = NO;
    } else {
        ...
        if (maxLength > 0) result =  MIN(defaultValue, (maxLength - bytesDone));
        ...
        if (shouldPreBufferPtr) {
            ...
            if (buffSpace >= result) *shouldPreBufferPtr = NO;
...
- (NSUInteger)readLengthForTermWithHint:(NSUInteger)bytesAvailable shouldPreBuffer:(BOOL *)shouldPreBufferPtr
{
    ...
    if (shouldPreBufferPtr) {
        ...
        if ((buffSize - buffUsed) >= result) *shouldPreBufferPtr = NO;
        else *shouldPreBufferPtr = YES;
    }
...
- (void)setDelegate:(id)newDelegate synchronously:(BOOL)synchronously
{
    ...
    if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
        ...
        if (synchronously) dispatch_sync(socketQueue, block);
        else dispatch_async(socketQueue, block);
    }
}
...
- (void)setIPv4Enabled:(BOOL)flag
{
    ...
    dispatch_block_t block = ^{
        if (flag) self->config &= ~kIPv4Disabled;
        else self->config |= kIPv4Disabled;
    };

    if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) block();
    else dispatch_async(socketQueue, block);
}
...
- (void)setIPv6Enabled:(BOOL)flag
{
    ...
    dispatch_block_t block = ^{
        ...
        else self->config |= kIPv6Disabled;
    };
...
- (void)setIPv4PreferredOverIPv6:(BOOL)flag
{
    ...
    dispatch_block_t block = ^{
        if (flag) self->config &= ~kPreferIPv6;
        else self->config |= kPreferIPv6;
    };
...
- (NSTimeInterval)alternateAddressDelay {
    ...
    if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) block();
    else dispatch_sync(socketQueue, block);
    return delay;
}
...
- (BOOL)doAccept:(int)parentSocketFD
{
    ...
    if (delegateQueue) {
        ...
        dispatch_async(delegateQueue, ^{ @autoreleasepool {
                                             ...
                                             if (socketType == 0) acceptedSocket->socket4FD = childSocketFD;
                                             else if (socketType == 1) acceptedSocket->socket6FD = childSocketFD;
                                             else acceptedSocket->socketUN = childSocketFD;
...
- (BOOL)bindSocket:(int)socketFD toInterface:(NSData *)connectInterface error:(NSError **)errPtr
{
    ...
    if (connectInterface) {
        ...
        if (result != 0) {
            if (errPtr) *errPtr = [self errorWithErrno:errno reason:@"Error in bind() function"];
...
- (int)createSocket:(int)family connectInterface:(NSData *)connectInterface errPtr:(NSError **)errPtr
{
    ...
    if (socketFD == SOCKET_NULL) {
        if (errPtr) *errPtr = [self errorWithErrno:errno reason:@"Error in socket() function"];
...
{
    if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
        if (socket4FD != SOCKET_NULL) return [self connectedHostFromSocket4:socket4FD];
        if (socket6FD != SOCKET_NULL) return [self connectedHostFromSocket6:socket6FD];
        ...
        dispatch_sync(socketQueue, ^{ @autoreleasepool {
                                          if (self->socket4FD != SOCKET_NULL) result = [self connectedHostFromSocket4:self->socket4FD];
                                          else if (self->socket6FD != SOCKET_NULL) result = [self connectedHostFromSocket6:self->socket6FD];
                                      } });
...
{
    if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
        if (socket4FD != SOCKET_NULL) return [self connectedPortFromSocket4:socket4FD];
        if (socket6FD != SOCKET_NULL) return [self connectedPortFromSocket6:socket6FD];
        ...
        dispatch_sync(socketQueue, ^{
            ...
            if (self->socket4FD != SOCKET_NULL) result = [self connectedPortFromSocket4:self->socket4FD];
            else if (self->socket6FD != SOCKET_NULL) result = [self connectedPortFromSocket6:self->socket6FD];
        });
...
{
    if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
        if (socketUN != SOCKET_NULL) return [self connectedUrlFromSocketUN:socketUN];
        ...
        dispatch_sync(socketQueue, ^{ @autoreleasepool {
                                          if (self->socketUN != SOCKET_NULL) result = [self connectedUrlFromSocketUN:self->socketUN];
                                      } });
...
{
    if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
        if (socket4FD != SOCKET_NULL) return [self localHostFromSocket4:socket4FD];
        if (socket6FD != SOCKET_NULL) return [self localHostFromSocket6:socket6FD];
        ...
        dispatch_sync(socketQueue, ^{ @autoreleasepool {
                                          if (self->socket4FD != SOCKET_NULL) result = [self localHostFromSocket4:self->socket4FD];
                                          else if (self->socket6FD != SOCKET_NULL) result = [self localHostFromSocket6:self->socket6FD];
                                      } });
...
{
    if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
        if (socket4FD != SOCKET_NULL) return [self localPortFromSocket4:socket4FD];
        if (socket6FD != SOCKET_NULL) return [self localPortFromSocket6:socket6FD];
        ...
        dispatch_sync(socketQueue, ^{
            ...
            if (self->socket4FD != SOCKET_NULL) result = [self localPortFromSocket4:self->socket4FD];
            else if (self->socket6FD != SOCKET_NULL) result = [self localPortFromSocket6:self->socket6FD];
        });
...
- (void)setupReadAndWriteSourcesForNewlyConnectedSocket:(int)socketFD
{
    ...
    dispatch_source_set_event_handler(readSource, ^{ @autoreleasepool {
        ...
        #pragma clang diagnostic warning "-Wimplicit-retain-self"
                                                         ...
                                                         if (strongSelf->socketFDBytesAvailable > 0) [strongSelf doReadData];
                                                         else [strongSelf doReadEOF];
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
    ...
    dispatch_block_t block = ^{
        if (!self->currentRead || ![self->currentRead isKindOfClass:[GCDAsyncReadPacket class]]) {
            ...
            if (total > 0) result = (float)done / (float)total;
            else result = 1.0F;
        }
    };
...
- (void)doReadData
{
    ...
    if ([self usingCFStreamForTLS]) {
        ...
        if ((flags & kSecureSocketHasBytesAvailable) && CFReadStreamHasBytesAvailable(readStream)) hasBytesAvailable = YES;
        else hasBytesAvailable = NO;
    ...
    if (!done && !error && !socketEOF && hasBytesAvailable) {
        ...
        if (flags & kSocketSecure) {
            if ([self usingCFStreamForTLS]) {
                ...
                if (result != noErr) {
                    if (result == errSSLWouldBlock) waiting = YES;
                    else {
            ...
            if (result < 0) {
                if (errno == EWOULDBLOCK) waiting = YES;
                else error = [self errorWithErrno:errno reason:@"Error in read() function"];
                ...
                if (bytesRead < bytesToRead) {
                    ...
                    if (socketFDBytesAvailable <= bytesRead) socketFDBytesAvailable = 0;
                    else socketFDBytesAvailable -= bytesRead;
                }
...
- (void)maybeStartTLS
{
    ...
    if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS)) {
                ...
                #if TARGET_OS_IPHONE
        {
            ...
            if (value && [value boolValue]) useSecureTransport = NO;
        }
                #endif
...
- (OSStatus)sslReadWithBuffer:(void *)buffer length:(size_t *)bufferLength
{
    ...
    if (sslPreBufferLength > 0) {
        ...
        size_t bytesToCopy;
        if (sslPreBufferLength > totalBytesLeftToBeRead) bytesToCopy = totalBytesLeftToBeRead;
        else bytesToCopy = sslPreBufferLength;
    ...
    if (!done && (socketFDBytesAvailable > 0)) {
        ...
        if (result < 0) {
            ...
            if (socketFDBytesAvailable > bytesReadFromSocket) socketFDBytesAvailable -= bytesReadFromSocket;
...
- (void)ssl_startTLS
{
    ...
    BOOL isServer = [isServerNumber boolValue];
        ...
        #if TARGET_OS_IPHONE || (__MAC_OS_X_VERSION_MIN_REQUIRED >= 1080)
    {
        if (isServer) sslContext = SSLCreateContext(kCFAllocatorDefault, kSSLServerSide, kSSLStreamType);
        else sslContext = SSLCreateContext(kCFAllocatorDefault, kSSLClientSide, kSSLStreamType);
...
+ (void)unscheduleCFReadStreams:(id)input
{
    ...
    NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
    CFRunLoopRef runLoop = CFRunLoopGetCurrent();
    if (readStream) CFReadStreamUnscheduleFromRunLoop(readStream, runLoop, kCFRunLoopDefaultMode);
}
...
+ (void)unscheduleCFWriteStreams:(id)input
{
    ...
    NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
    CFRunLoopRef runLoop = CFRunLoopGetCurrent();
    if (writeStream) CFWriteStreamUnscheduleFromRunLoop(writeStream, runLoop, kCFRunLoopDefaultMode);
}
    ...
    switch (type) {
        case kCFStreamEventHasBytesAvailable:{
            dispatch_async(asyncSocket->socketQueue, ^{ @autoreleasepool {
                                                            ...
                                                            if (asyncSocket->readStream != stream) return_from_block;
    ...
    switch (type) {
        case kCFStreamEventCanAcceptBytes:{
            dispatch_async(asyncSocket->socketQueue, ^{ @autoreleasepool {
                                                            ...
                                                            if (asyncSocket->writeStream != stream) return_from_block;
...
- (BOOL)createReadAndWriteStream
{
    ...
    if (readStream) CFReadStreamSetProperty(readStream, kCFStreamPropertyShouldCloseNativeSocket, kCFBooleanFalse);
    if (writeStream) CFWriteStreamSetProperty(writeStream, kCFStreamPropertyShouldCloseNativeSocket, kCFBooleanFalse);
...
- (BOOL)registerForStreamCallbacksIncludingReadWrite:(BOOL)includeReadWrite
{
    ...
    if (includeReadWrite) readStreamEvents |= kCFStreamEventHasBytesAvailable;
    ...
    if (includeReadWrite) writeStreamEvents |= kCFStreamEventCanAcceptBytes;
...
- (void)setAutoDisconnectOnClosedReadStream:(BOOL)flag
{
    ...
    dispatch_block_t block = ^{
        if (flag) self->config &= ~kAllowHalfDuplexConnection;
        else self->config |= kAllowHalfDuplexConnection;
    };
...
- (int)socketFD
{
    ...
    if (socket4FD != SOCKET_NULL) return socket4FD;
    else return socket6FD;
}
...
- (CFReadStreamRef)readStream
{
    ...
    if (readStream == NULL) [self createReadAndWriteStream];
...
- (CFWriteStreamRef)writeStream
{
    ...
    if (writeStream == NULL) [self createReadAndWriteStream];
...
+ (NSString *)hostFromAddress:(NSData *)address
{
    ...
    if ([self getHost:&host port:NULL fromAddress:address]) return host;
    else return nil;
}
...
+ (uint16_t)portFromAddress:(NSData *)address
{
    ...
    if ([self getHost:NULL port:&port fromAddress:address]) return port;
    else return 0;
}
